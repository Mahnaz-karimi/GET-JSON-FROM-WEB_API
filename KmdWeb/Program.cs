...
namespace KmdWeb
{
    public class Program
    {
        ...
        public static Double get_new_IntervalInt_is_On_time(Double jsont_difference_from_now)
        {
        ...
        {           
            return Convert.ToDouble((DateTime.Now - update_At).TotalMilliseconds);
        }

        public static void getNewTimer(Double jsont_difference_from_now) //  get new timer time for events.
        {
            if (jsont_difference_from_now <= interval_event) // if the time difference between json and now is less or equal to the time that is estimated, we get timer under 3 min
            {
                newTimer.Interval = get_new_IntervalInt_is_On_time(jsont_difference_from_now);           
        ...
        public static void update_sql_TimerEvent(object source, ElapsedEventArgs e)
        {
            ...
            Double difference_Json_SQl = getDifference_Json_SQl(update_At);
            save_In_Sql_If_There_Is_Difference(difference_Json_SQl, json);
            
            // when the program is here, it means difference Json SQL is zero and then we get new timer time for update         
            getNewTimer(getDifference_Json_Now(update_At)); // get difference from json and now and calculate interval
